...
<!-- <div class="q-pa-md"> -->
    <q-layout view="lHh Lpr lff" >
      <q-header reveal class="bg-white text-dark q-ma-md" >
        <template v-if="user_data[chatting_with_uuid]">                     
        <q-toolbar style="border: 1px solid #ddd" class="radius10 q-pa-sm" >            
          <q-avatar size="55px">           
           <q-img
                :src="user_data[chatting_with_uuid].photo_url"
          ...
          <q-toolbar-title>
            <div>  
              <h6 class="q-ma-none q-pa-none line-normal">{{user_data[chatting_with_uuid].first_name}} {{user_data[chatting_with_uuid].last_name}}</h6>
              <p class="q-ma-none text-grey q-pa-none font13">{{user_data[chatting_with_uuid].contact_phone}}</p>
            </div>
          </q-toolbar-title>
          <div>                         
             ...
             <!-- <q-btn round color="dark" flat icon="more_vert" >
                 <q-popup-proxy style="min-width: 150px;">
                  <q-list>
                      <q-item clickable v-ripple :href="user_data[chatting_with_uuid].profile_url" target="_blank">
                          <q-item-section><?php echo t("View Profile")?></q-item-section>
                      </q-item>
                      <q-item clickable v-ripple @click="deleteChat" >
        ...
        <template v-else>
           <q-toolbar style="border: 1px solid #ddd" class="radius10 q-pa-sm" >
               ...
               <div class="xs">
                  <q-btn round color="dark" flat icon="arrow_back" href="<?php echo Yii::app()->createUrl("/account/profile")?>" ></q-btn>
               </div>
               <div>
                  <q-btn round color="dark" flat icon="menu" @click="drawer=!drawer" ></q-btn>
               </div>
              ...
              <template v-if="hasSearchData && !search_loading && hasSearch">                
                <q-list class="list-custom">
                   <template v-for="items in getSearchData" :key="items">
                      <q-item clickable v-ripple @click="chatToUser(items.client_uuid)"> 
