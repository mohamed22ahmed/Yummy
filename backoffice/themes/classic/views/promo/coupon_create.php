...
<div class="row mt-4" id="switch_type">
...
<div class="mt-4" id="discount_delivery_selectbox">
    ...
    <div class="row" id="switchWhenSelectDeliveryCostPayerAdmin">
        <div class="col-md-6 form-label-group">
        ...
        <div class="col-md-6" id="switchWhenSelectDeliveryCostPayerIsSharedAdmin">
            <div class="row">
                <div class="col-md-6 form-label-group">
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
      ...
      const voucher_type = document.querySelector('.voucher_type');
      const switch_type = document.getElementById('switch_type');
      const amount = switch_type.querySelector('.switch_type_amount');
      const min_order = switch_type.querySelector('.switch_type_min_order');
      const up_to = switch_type.querySelector('.switch_type_up_to');

      changeClassesWhenChangingCouponType(voucher_type.value)
      voucher_type.addEventListener('change', function () {
          changeClassesWhenChangingCouponType(voucher_type.value)
      });


      // select Delivery Cost Payer
      const delivery_cost_payer_merchantAdmin = document.querySelector('.delivery_cost_payer');
      const switchWhenSelectDeliveryCostPayerAdmin = document.getElementById('switchWhenSelectDeliveryCostPayerAdmin');
      const switchWhenSelectDeliveryCostPayerIsSharedAdmin = document.getElementById('switchWhenSelectDeliveryCostPayerIsSharedAdmin');

      changeClassesWhenChangingDeliveryCostPayer(delivery_cost_payer_merchantAdmin.value)
      delivery_cost_payer_merchantAdmin.addEventListener('change', function () {
          changeClassesWhenChangingDeliveryCostPayer(delivery_cost_payer_merchantAdmin.value)
      });
      ...
      function changeClassesWhenChangingCouponType(type) {
          if(type === "fixed amount") {
              amount.classList.remove('col-md-4');
              min_order.classList.remove('col-md-4');
              up_to.classList.remove('col-md-4');

              amount.classList.add('col-md-6');
              min_order.classList.add('col-md-6');
              up_to.style.display = 'none';
          } else {
              amount.classList.remove('col-md-6');
              min_order.classList.remove('col-md-6');

              amount.classList.add('col-md-4');
              min_order.classList.add('col-md-4');
              up_to.classList.add('col-md-4');
              up_to.style.display = 'block';
          }
      }

      function changeClassesWhenChangingDeliveryCostPayer(type) {
          if (type != 1) {
              switchWhenSelectDeliveryCostPayerAdmin.style.display = 'block';
              if(type == 3) {
                  switchWhenSelectDeliveryCostPayerIsSharedAdmin.style.display = 'block';
              }else{
                  switchWhenSelectDeliveryCostPayerIsSharedAdmin.style.display = 'none';
              }
          }else {
              switchWhenSelectDeliveryCostPayerAdmin.style.display = 'none';
          }
      }
  });
</script>
