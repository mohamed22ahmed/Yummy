...
var empty = function(e) {
	if (typeof e === "undefined" || e == null || e == "" || e == "null" || e == "undefined") {
		return true
	}
	return false
};
...
const quasarComponents = {
	...
	setStorage(e, t) {
		try {
			Quasar.LocalStorage.set(e, t)
		} catch (s) {
			console.debug(s)
		}
	},
	getStorage(e) {
		return Quasar.LocalStorage.getItem(e)
	},
	notify(e, t, s) {
		const a = Quasar.Notify;
		a.create({
			message: t,
			color: e,
			icon: s,
...
const componentsParticipants = {
	...
	methods: {
		getParticipants() {
			this.loading = true;
			const e = collection(firebaseDb, firebaseCollectionEnum.chats);
			const t = query(e, where("participants", "array-contains", this.user_uuid), orderBy("lastUpdated", "desc"), limit(firebaseCollectionEnum.limit));
			const s = onSnapshot(t, e => {
				this.data = [];
				this.users = [];
				this.all_users = [];
				this.loading = false;
				e.forEach(s => {
					let a = s.data();
					let i = a.isTyping || null;
					let r = a.participants || null;
					if (r) {
						if (Object.keys(r).length > 0) {
							Object.entries(r).forEach(([e, t]) => {
								this.all_users.push(t)
							})
						}
						let e = r.filter(e => !e.includes(this.user_uuid));
						let t = e[0] ? e[0] : null;
						this.users.push(t);
						this.data.push({
							doc_id: s.id,
							user_uuid: t,
							is_typing: i[e[0]] ? i[e[0]] : false,
							orderID: a.orderID || null,
							orderUuid: a.orderUuid || null
						})
					}
				});
				...
				this.loading = false;
				console.log("Error fetching chat documents:", e)
			})
		},
		getUser() {
			...
			axios({
				...
				if (e.data.code == 1) {
					this.users_data = e.data.details
				} else {
					this.users_data = []
				}
		...
		async getLastMessage() {
			try {
				if (Object.keys(this.all_users).length > 0) {
					const i = this.all_users.splice(0, 10);
					const t = collection(firebaseDb, firebaseCollectionEnum.chats);
					const s = await getDocs(query(t, where("participants", "array-contains-any", i)));
					s.forEach(async e => {
						const a = e.id;
						const t = collection(firebaseDb, firebaseCollectionEnum.chats, a, "messages");
						const s = await getDocs(query(t, where("senderID", "in", i), orderBy("timestamp", "desc"), limit(1)));
						s.forEach(e => {
							let t = e.data();
							let s = t.timestamp.toDate().toISOString();
							this.last_message_data[a] = {
								message: t.message,
								timestamp: s,
								time: DateTime.fromISO(s).toFormat("hh:mm a")
							}
						})
					})
				}
		...
		async getWhoIsTyping() {
			if (Object.keys(this.all_users).length > 0) {
				const e = this.all_users.splice(0, 10);
				const t = query(collection(firebaseDb, firebaseCollectionEnum.chats), where("participants", "array-contains-any", e), orderBy("lastUpdated", "desc"), limit(firebaseCollectionEnum.limit));
				const s = onSnapshot(t, e => {
					e.forEach(e => {
						let t = e.data();
						let s = t.isTyping || [];
		...
		isTyping(t) {
			if (Object.keys(this.whoistyping_data).length > 0) {
				let e = this.whoistyping_data[t] || false;
				return e
			}
			return false
		},
		onClickChat(e) {
			this.document_id = e;
			this.$emit("afterClickconversation", e)
		}
	},
	template: `            
...
const componentsMessages = {
	...
	watch: {
		conversation_id(e, t) {
			if (!empty(e)) {
				this.getMessages(e);
				this.getWhoIsTyping(e);
				this.getParticipant(e)
			} else {
				this.data = [];
	...
	methods: {
		getMessages(e) {
			this.loading = true;
			const t = doc(firebaseDb, firebaseCollectionEnum.chats, e);
			const s = query(collection(t, "messages"), orderBy("serverTimestamp", "asc"), limit(firebaseCollectionEnum.limit));
			const a = onSnapshot(s, e => {
				this.data = [];
				this.loading = false;
				e.forEach(t => {
					if (t.exists()) {
						const s = t.data();
						let e = s.timestamp.toDate().toISOString();
						this.data.push({
							fileType: s.fileType,
							fileUrl: s.fileUrl,
							message: s.message,
							senderID: s.senderID,
							time: DateTime.fromISO(e).toFormat("ccc hh:mm a")
						})
					} else {
		...
		getWhoIsTyping(e) {
			const t = doc(firebaseDb, firebaseCollectionEnum.chats, e);
			const s = onSnapshot(t, t => {
				if (t.exists()) {
					let e = t.data();
					this.user_typing_data = e.isTyping || []
				} else {
					this.user_typing_data = []
				}
				this.$emit("scrollTobottom")
			}, e => {
				console.error("Error fetching chat document:", e)
			})
		},
		async getParticipant(e) {
			try {
				const s = doc(firebaseDb, firebaseCollectionEnum.chats, e);
				const a = await getDoc(s);
				if (a.exists()) {
					const i = a.data().participants;
					let e = i.filter(e => !e.includes(this.user_uuid));
					this.chating_with_user_uuid = e[0] ? e[0] : null;
					this.$emit("setChattingwith", this.chating_with_user_uuid)
				} else {
					console.log("Conversation document does not exist");
					this.$emit("setChattingwith", null)
				}
			} catch (t) {
				console.error("Error getting participants:", t)
			}
		}
	},
	template: `                     
...
const componentsChat = {
	...
	methods: {
		...
		async saveChatMessage() {
			this.loading = true;
			const e = collection(firebaseDb, firebaseCollectionEnum.chats, this.conversation_id, "messages");
			try {
				await addDoc(e, {
				...
				console.error("Error adding message to the conversation:", t);
				quasarComponents.notify("red-5", t, "error_outline")
			}
		},
		async documentLastUpdate(e) {
			try {
				const s = doc(firebaseDb, firebaseCollectionEnum.chats, e);
				await updateDoc(s, {
		...
		afterUploaded(i) {
			if (i.xhr.status == 200) {
				let e = JSON.parse(i.xhr.response);
				let t = e.code || false;
				let s = e.details || [];
				let a = e.msg || "";
				if (t == 1) {
					this.file_url = s.file_url;
					this.file_type = s.file_type;
					this.saveChatMessage()
				} else {
					quasarComponents.notify("red-5", a, "error_outline");
					this.$refs.uploader.reset()
				}
			} else {
		...
		async UpdateWhoistyping(e) {
			try {
				const s = doc(firebaseDb, firebaseCollectionEnum.chats, this.conversation_id);
				await updateDoc(s, {
					[`isTyping.${this.user_uuid}`]: e
				})
			} catch (t) {
				console.error("Error updating typing status:", t)
			}
		}
	},
	template: `   
...
const componentsSearchChat = {
	...
	watch: {
		awaitingSearch(e, t) {
			this.$emit("onSearchloading", e)
		},
		is_search(e, t) {
			this.$emit("onSearchchat", e)
		},
		search(e, t) {
			this.$emit("setSearchtext", e);
			if (!this.awaitingSearch) {
				...
				setTimeout(() => {
					axios({
						...
						if (e.data.code == 1) {
							this.$emit("onSearchresults", e.data.details)
						} else {
...
const deleteMessagesInConversation = async e => {
	try {
		const s = collection(firebaseDb, firebaseCollectionEnum.chats, e, "messages");
		const a = await getDocs(s);
		a.forEach(async e => {
			await deleteDoc(e.ref)
		});
		console.log("All messages in the conversation deleted successfully.")
	} catch (t) {
		console.error("Error deleting messages in conversation:", t)
	}
};
const app_chat = Vue.createApp({
	...
	methods: {
		...
		setWhoistyping(e) {
			this.user_typing_data = e
		},
		afterClickconversation(e) {
			this.conversation_id = e
		},
		setChattingwith(e) {
			this.chatting_with_uuid = e;
			this.scrollTobottom()
		},
		afterAddmessage() {
			this.scrollTobottom();
		...
		scrollTobottom() {
			setTimeout(() => {
				if (typeof this.$refs.scroll_ref !== "undefined" && this.$refs.scroll_ref !== null) {
					let e = parseInt(this.$refs.scroll_ref.getScroll().verticalSize) + 100;
					this.$refs.scroll_ref.setScrollPosition("vertical", e)
				}
			}, 500)
		},
		onSearchchat(e) {
		...
		async chatToUser(e) {
			try {
				const s = collection(firebaseDb, firebaseCollectionEnum.chats);
				const i = query(s, where("participants", "array-contains", e), orderBy("lastUpdated", "desc"), limit(1));
				let a = "";
				const r = await getDocs(i);
				r.forEach(e => {
					let t = e.data();
					let s = t.participants || null;
					if (s.includes(main_user_uuid) === true) {
						a = e.id
					}
				});
				console.log("main_user_uuid=>" + main_user_uuid);
				console.log("chatToUser=>" + e);
				console.log("current_doc_id=>" + s);
				if (!empty(a)) {
					this.loadConversation(a, e)
				} else {
      ...
      try {
        // Concatenate participantId and main_user_uuid to form a unique document ID
        const docId = `${participantId}_${main_user_uuid}`;
    
        // Create a document reference with the composite ID
        const docRef = doc(firebaseDb, firebaseCollectionEnum.chats, docId);
    
        // Define the conversation data
        const conversationData = {
          lastUpdated: serverTimestamp(),
          dateCreated: serverTimestamp(),
          participants: [participantId, main_user_uuid],
          isTyping: {
            [participantId]: false,
        ...
        this.loadConversation(docId);
      } catch (error) {
        // Handle any errors
        quasarComponents.notify("red-5", error.message, "error_outline");
      }
    },
		loadConversation(e) {
			...
			setTimeout(() => {
				if (typeof this.$refs.participants !== "undefined" && this.$refs.participants !== null) {
					this.$refs.participants.document_id = e
				}
			}, 600)
		},
		async deleteChat() {
			try {
				const t = doc(firebaseDb, firebaseCollectionEnum.chats, this.conversation_id);
        
				await deleteDoc(t);
				let e = this.conversation_id;
				this.conversation_id = "";
				deleteMessagesInConversation(e)
			} catch (e) {
				quasarComponents.notify("red-5", e, "error_outline")
			}
		},
		notifyUser() {
			axios({
				...
				data: {
					user_uuid: this.chatting_with_uuid,
					from_user_uuid: main_user_uuid
				}
			}).then(e => {})["catch"](e => {}).then(e => {})
		}
	}
});
app_chat.use(Quasar, {
