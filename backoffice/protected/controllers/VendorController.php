...
class VendorController extends CommonController
{
	...
	public function actionEdit()
	{
		...
		if($id>0){
			...
			$model->ar_restaurant_name = stripslashes($model->ar_restaurant_name);
    ...
    public function actionlogin()
    {
    	...
    	if($merchant){
			...
			if(Yii::app()->params['isMobile']==TRUE){
               ...
               $menu->isEntity = $merchant->isEntity;
	...
	public function actionzone()
	{
		...
		if($model){
            ...
            $where_statement_for_getting_zones = "where merchant_id=0";
            if(isset($model->city_id) && !empty($model->city_id)){
                $where_statement_for_getting_zones = "where merchant_id=0 AND mode = 'delivery' AND city_id=". $model->city_id;
            }
			...
			$params_model = array(		
				...
				'zone_list'=>CommonUtility::getDataToDropDown("{{zones}}",'zone_id','zone_name',$where_statement_for_getting_zones,"Order by zone_name asc"),
			);
	...
	public function actiontableside_settings()
	{
		...
		if($merchant){
			...
			$service_list = AttributesTools::ListSelectServices();
