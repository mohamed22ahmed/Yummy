...
class Commonmerchant extends CController
{
	...
	public function __construct($id,$module=null){
		...
		if(isset($_GET['setlang'])) {
			...
			Yii::app()->cache->delete('cache_merchant_search_menu');
		}
	...
	{						
		return array(			
		    array('allow',
                'actions'=>array('logout','error','access_denied','profile','change_password','profile_remove_image','plan','web_notifications'),
                'users'=>array('*'),                
            ),
			array('allow', 			    
	...
	public function init()
	{
		...
		/* add rtl 2024-23-9 mazen */
		if($is_rtl){
			$include = array('backend-core-rtl','backend-css','merchant-js');
		} else $include = array('backend-core','backend-css','merchant-js');
		/* add rtl 2024-23-9 mazen */
	...
	//	$include = array('backend-core','backend-css','merchant-js');
		...
		if(Yii::app()->controller->id=="communications"){
			...
			if (preg_match("/createorder/i",Yii::app()->request->requestUri)) {
				$include = [];
				$include[]='pusher';
				$include[]='quasar';
				$include[]='pos';
		...
		// dump(Yii::app()->language);
		// dump($translation_vendor);die();
	...
	public function initSettings()
	{
		Yii::app()->params['settings'] = OptionsTools::find(array(
			  ...
			  'yandex_geosuggest_api','yandex_geocoder_api','yandex_static_api','yandex_distance_api'
		));
