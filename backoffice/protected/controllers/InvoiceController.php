...
class InvoiceController extends CommonController
{
    ...
    public function actionview()
    {
        try {
            ...
            $createdDate = $model->date_created; // Example date, replace with your actual created date

            // Create a DateTime object from the created date
            $invoiceDate = new DateTime($createdDate);

            // Add 15 months to the invoice date
            $invoiceDate->modify('+15 months');

            // Get the new date after modification
            $newInvoiceDate = $invoiceDate->format('Y-m-d');

            $months = 15; // Number of months for the period
            

            $countOfOrders = AR_ordernew::getOrdersCountInPeriod((new DateTime($createdDate))->format('Y-m-d'), $months,$model->merchant_id);
            
            $totalPriceOfflinePaid = AR_ordernew::getTotalPriceOfflinePaid($model->merchant_id);
            $totalPriceOnlinePaid = AR_ordernew::getTotalPriceOnlinePaid($model->merchant_id);
            $total = $totalPriceOfflinePaid + $totalPriceOnlinePaid;
            // $commission_value = AR_ordernew::getCommissionByMerchantId($model->merchant_id);
            // var_dump($model->merchant_id);
            // exit;
             $resp_comm = CMerchants::getCommissionData($model->merchant_id);

            $pickup = $resp_comm['commission_value']['pickup'];
            $dinein = $resp_comm['commission_value']['dinein'];
            $total_rate = $pickup + $dinein;

            $sumNegativeAdjustmentTotal = AR_ordernew::getSumNegativeAdjustmentTotal($model->merchant_id);
            $sumPositiveAdjustmentTotal = AR_ordernew::getSumPositiveAdjustmentTotal($model->merchant_id);
            $totalmodifications = $sumNegativeAdjustmentTotal + $sumPositiveAdjustmentTotal;
            $merchant_credit = $total + $totalmodifications;


            $totalAmountDue = AR_ordernew::getTotalAmountDueByMerchant($model->merchant_id);
            ...
            $this->render("invoice_view", [
                ...
                'totalPriceOfflinePaid'  => $totalPriceOfflinePaid,
                'totalPriceOnlinePaid'  => $totalPriceOnlinePaid,
                'total'  => $total,
                'pickup'  => $pickup,
                'dinein'  => $dinein,
                'total_rate'  => $total_rate,
                'sumNegativeAdjustmentTotal'  => $sumNegativeAdjustmentTotal,
                'sumPositiveAdjustmentTotal'  => $sumPositiveAdjustmentTotal,
    ...
    public function actioncreate($update = false)
    {
        ...
        if ($update) {
            ...
            $merchant_selected = CommonUtility::getDataToDropDown(
                "{{merchant}}",
                'merchant_id',
                'restaurant_name',
    ...
    public function actionsettings()
    {
        ...
        $template_list =  CommonUtility::getDataToDropDown(
            "{{templates}}",
            'template_id',
            'template_name',
            "",
