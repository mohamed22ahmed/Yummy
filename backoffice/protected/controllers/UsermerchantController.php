...
class UsermerchantController extends Commonmerchant
{
	...
	public function actionuser_create($update=false)
	{
		...
		if($update){
			...
			$model = AR_merchant_user::model()->with('merchants')->find("t.merchant_id=:merchant_id AND t.user_uuid=:user_uuid",array(
		...
		if(isset($_POST['AR_merchant_user'])){
            ...
            $merchant_ids = isset($_POST['AR_merchant_user']['merchant_ids']) ? $_POST['AR_merchant_user']['merchant_ids'] : array();
            ...
            if($model->validate()){
				...
				if($model->save()){
                    $this->saveMerchantSharedLocation($model, $merchant_ids);
    ...
    protected function saveMerchantSharedLocation($model, $merchantIds)
    {
        // Clear existing relations
        Yii::app()->db->createCommand()->delete('{{user_shared_merchant}}', 'merchant_user_id=:merchant_user_id', array(':merchant_user_id' => $model->merchant_user_id));

        // Add new relations
        foreach ($merchantIds as $child_merchant_id) {
            Yii::app()->db->createCommand()->insert('{{user_shared_merchant}}', array(
	...
	public function actionrole_create($update=false)
	{
		...
		AdminMenu::buildMenu(0,true,0,'merchant');			
