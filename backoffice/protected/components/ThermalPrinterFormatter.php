    ...
    public static function RawReceipt()
    {
       ...
       if($payment_change>0){
            $body[]=[
                ...
                'value'=>$print_type=="raw"?$orderinfo['payment_change']:$orderinfo['payment_change_pretty']    
            ];    
       }
       ...
       if(is_array($items) && count($items)>=1){
         foreach ($items as $items) {                   
             ...
             $body[] = [
                ...
                'value'=>$print_type=="raw"?$items['price']['total_after_discount']:$items['price']['pretty_total_after_discount']
             ];
                    ...
                    if(is_array($addon_items) && count($addon_items)>=1){
                        foreach ($addon_items as $addon_items_key => $addon_items_val) { 
                            $body[] = [
                                ...
                                'value'=>$print_type=="raw"?$addon_items_val['addons_total']:$addon_items_val['pretty_addons_total']
                             ];
                        }
                    }
                }
       ...
       if(is_array($summary) && count($summary)>=1){
            foreach ($summary as $items) {                 
                ...
                $body[] = [
                    ...
                    'type'=>"text",
                    'label'=>$items['name'],
                    'value'=>$print_type=="raw"?$items['raw']:$items['value']
                ];
            }        
       }
