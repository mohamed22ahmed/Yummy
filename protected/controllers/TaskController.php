...
class TaskController extends SiteCommon
{
	...
	{	    
		...
		$points_enabled = isset(Yii::app()->params['settings']['points_enabled'])?Yii::app()->params['settings']['points_enabled']:false;
		$points_enabled = $points_enabled==1?true:false;			
		$points_first_order = isset(Yii::app()->params['settings']['points_first_order'])? floatval(Yii::app()->params['settings']['points_first_order']):0;
	...
	public function autoassign()
	{
		try {
			...
			$options = OptionsTools::find(['driver_enabled_auto_assign','driver_assign_when_accepted','driver_prep_check_when_assign','driver_assign_prep_time','driver_allowed_number_task','driver_on_demand_availability']);
            ...
            if($driver_prep_check_when_assign){
                Yii::log( "driver_prep_check_when_assign", CLogger::LEVEL_ERROR);

                if($driver_assign_prep_time > $order->prep_time){
                    Yii::log( "the assign preparation time should be less than or equal to the order preparation time" , CLogger::LEVEL_ERROR);
                    die("the assign prep time is greater than the order prep time");
                }
            }
			...
			if(!$on_demand_availability){				
				...
				* cos( radians( lontitude ) - radians($filter[longitude]) ) 
				...
				HAVING ".$allowed_number_task." > active_task
				ORDER BY active_task ASC, total_delivered ASC 
				LIMIT 0,5	
				";				
			}
			dump($stmt);									
	...
	public function afterdelivered()
	{
		try {
			...
			// dump("commission_per_delivery=>$commission_per_delivery");
	...
	public function actiongetneworder()
	{
		try {
			...
			$new_status = COrders::newOrderStatus();			
			$meta_name = 'last_process_id';
			$date_created = date("Y-m-d");						
			
			$last_process = AR_admin_meta::getMeta([$meta_name]);			
			$last_process_id = isset($last_process['last_process_id'])?$last_process['last_process_id']['meta_value']:0;			
			$all_order_process = $this->areAllOrdersProcessed($last_process_id);			
			if($all_order_process){							
				$this->resetLastProcessedOrderId($meta_name);
			}			
			...
			$criteria=new CDbCriteria();
			$criteria->addCondition("is_view=:is_view AND status=:status 
			AND order_id > ".q($last_process_id)."
			AND date_created > now() - interval 30 minute");			
			$criteria->params = [
				':is_view'=>0,
				':status'=>$new_status,				
			];			
			//AND DATE_FORMAT(date_created,'%Y-%m-%d')=:date_created");
