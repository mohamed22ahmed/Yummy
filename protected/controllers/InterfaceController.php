...
{
    private $secretKey = CYBERSOURCE_SECRET_KEY;
    private $accessKey = CYBERSOURCE_ACCESS_KEY;
    private $profileId = CYBERSOURCE_PROFILE_ID;
    ...
    public function actionCybersourcePaymentPayload()
    {
        $postdata = [
            ...
            'currency' => $this->data['currency'] ?? 'ILS',
        ];

        $postdata['signature'] = $this->generateSignature($postdata);

        $checkoutUrl = 'https://testsecureacceptance.cybersource.com/pay?' . http_build_query($postdata);

        $this->code = 1;
        $this->msg = 'got url to redirect';
        $this->details = [
            'checkoutUrl' => $checkoutUrl,
            'transactionId' => $this->data['order_id'] ?? 0,
        ];

        $this->responseJson();
    }

    public function actioncybersourcePaymentResponse()
    {
        $transactionId = $this->data['transaction_id'] ?? '';
        $paymentStatus = $this->data['payment_status'] ?? 'AUTHORIZED';
        $signature = $this->data['signature'] ?? '';

        $this->code = 2;
        if ($this->validateResponse($signature)) {
            ...
            $this->msg = 'Invalid response from CyberSource.';
        }

        $this->responseJson();
    }

    private function validateResponse($signature)
    {
        return true;
    }
    ...
    public function actiongetMerchantFeed()
    {
        try {
            ...
            $store_type = isset($this->data['store_type']) ? $this->data['store_type'] : '';
            $view_type = isset($this->data['view_type']) ? strtoupper($this->data['view_type']) : '';

            $valid_store_types = ['RESTAURANT', 'SUPERMARKET', 'HOME_COOKING', 'CHAIN', 'SUPERMARKET_CHAIN', 'STORE_CHAIN'];
            if (!empty($store_type)) {
                if (is_array($store_type)) {
                    foreach ($store_type as $type) {
                        if (!in_array($type, $valid_store_types)) {
                            throw new Exception("Invalid store type: $type");
                        }
                    }
                } else {
                    if (!in_array($store_type, $valid_store_types)) {
                        throw new Exception("Invalid store type: $store_type");
                    }
                }
            }
            ...
            if(isset($place_data['latitude']) && isset($place_data['longitude'])){
                $point = ['lat' => $place_data['latitude'], 'lon' => $place_data['longitude']];
            ...
            if (!empty($store_type)) {
                if (is_array($store_type)) {
                    $store_type_list = "'" . implode("','", array_map('addslashes', $store_type)) . "'";
                    $filters['condition'] .= " AND (a.type IN ($store_type_list) OR 
                                              (a.parent_id IS NOT NULL AND 
                                               EXISTS (SELECT 1 FROM st_merchant AS parent 
                                                       WHERE parent.merchant_id = a.parent_id 
                                                       AND parent.type IN ($store_type_list))))";
                } else {
                    $filters['condition'] .= " AND (a.type = :store_type OR 
                                              (a.parent_id IS NOT NULL AND 
                                               EXISTS (SELECT 1 FROM st_merchant AS parent 
                                                       WHERE parent.merchant_id = a.parent_id 
                                                       AND parent.type = :store_type)))";
                    $filters['params'][':store_type'] = $store_type;
                }
            }

            if (!empty($view_type)) {
                $filters['condition'] .= " AND a.view_type = :view_type";
                $filters['params'][':view_type'] = $view_type;
            }
    ...
    public function actiongetDeliveryDetails()
    {
        try {
            ...
            // Determine the merchant ID based on query type
            if ($query_type == "cart") {
            ...
            // Check if the merchant is part of a chain
            $merchant = CMerchants::get($merchant_id); // Fetch merchant details
            $is_chain = in_array($merchant['type'], ['CHAIN', 'SUPERMARKET_CHAIN', 'STORE_CHAIN']);
            $is_child = ($merchant['type'] == 'CHILD');

            // Get the chain ID (either the merchant itself or its parent)
            $chain_id = ($is_chain) ? $merchant['merchant_id'] : ($is_child ? $merchant['parent_id'] : null);

            // If it's a CHILD, ensure the parent exists
            if ($is_child && empty($chain_id)) {
                throw new Exception("Child merchant does not belong to a chain.");
            }

            // Get all merchant IDs to search (include the current merchant and its chain/children)
            $merchant_ids_to_search = [$merchant_id];
            if ($chain_id) {
                $merchant_ids_to_search = array_merge($merchant_ids_to_search, CMerchants::getChildrenByChainId($chain_id));
            }

            // Fetch transaction details for all relevant merchants
            $data = [];
            foreach ($merchant_ids_to_search as $id) {
                try {
                    $merchant_data = CCheckout::getMerchantTransactionList($id, Yii::app()->language);
                    $data = array_merge($data, $merchant_data);
                } catch (Exception $e) {
                    // If no data found for a specific merchant, skip it
                    continue;
                }
            }

            // Fallback to default service listing if no data is found
            if (empty($data)) {
                $data = CServices::Listing(Yii::app()->language);
            }

            // Ensure the selected transaction type exists in the data
            if (!array_key_exists($transaction_type, (array)$data)) {
                $transaction_type = CServices::getSetService($cart_uuid);
            }

            // Save the transaction type to the cart
    ...
    public function actiongetMerchantInfo()
    {
        try {
            ...
            // If it's a CHAIN or CHILD, fetch additional chain-related data
            if ($chain_id) {
                $children = CMerchants::getChildrenByChainId($chain_id); // Get child merchants
                $data['chain_children'] = $children; // Add child merchants to the response
            }

            // Opening hours
            $opening_hours = CMerchantListingV1::openingHours($merchant_id);

            // Merchant timezone and currency settings
            ...
            $merchant_timezone = isset($options_merchant['merchant_timezone']) ? $options_merchant['merchant_timezone'] : '';
            $booking_enabled = isset($options_merchant['booking_enabled']) ? ($options_merchant['booking_enabled'] == 1 ? true : false) : false;
            $captcha_enabled = isset($options_merchant['booking_enabled_capcha']) ? ($options_merchant['booking_enabled_capcha'] == 1 ? true : false) : false;
            ...
            $allowed_choose_table = isset($options_merchant['booking_allowed_choose_table']) ? ($options_merchant['booking_allowed_choose_table'] == 1 ? true : false) : false;

            $merchant_default_currency = isset($options_merchant['merchant_default_currency']) ? $options_merchant['merchant_default_currency'] : '';
            $merchant_default_currency = !empty($merchant_default_currency) ? $merchant_default_currency : Price_Formatter::$number_format['currency_code'];
            ...
            $currency_code = !empty($currency_code) ? $currency_code : (!empty($merchant_default_currency) ? $merchant_default_currency : $base_currency);
            ...
            $charge_type = OptionsTools::find(['merchant_delivery_charges_type'], $merchant_id);
            ...
            try {
                $place_data = CMaps::locationDetails($place_id, '');
                $distance = CMaps::getLocalDistance($data['distance_unit'], $place_data['latitude'], $place_data['longitude'], $data['latitude'], $data['lontitude']);
            } catch (Exception $e) {
                // Ignore
            }
            ...
            // Map directions
            $map_direction = CMerchantListingV1::mapDirection($maps_config_raw, $data['latitude'], $data['lontitude']);
    ...
    public function actiongeStoreMenu()
    {
        try {
            ...
            // Fetch merchant details by slug
            $model = CMerchantListingV1::getMerchantBySlug($slug);
            $merchant_id = $model->merchant_id;

            // Determine if the merchant is a CHILD and get the parent ID if applicable
            $merchant = CMerchants::get($merchant_id); // Fetch merchant details
            $is_child = ($merchant['type'] == 'CHILD');
            $parent_id = ($is_child) ? $merchant['parent_id'] : null;

            // Use the parent ID if the merchant is a CHILD, otherwise use the original merchant ID
            $effective_merchant_id = ($is_child && !empty($parent_id)) ? $parent_id : $merchant_id;

            // CHECK IF MERCHANT HAS DIFFERENT TIMEZONE
            $options_merchant = OptionsTools::find(['merchant_timezone', 'merchant_default_currency'], $effective_merchant_id);
            $merchant_timezone = isset($options_merchant['merchant_timezone']) ? $options_merchant['merchant_timezone'] : '';
            $merchant_default_currency = isset($options_merchant['merchant_default_currency']) ? $options_merchant['merchant_default_currency'] : '';
            ...
            // Fetch menu data using the effective merchant ID
            $category = CMerchantMenu::getCategory($effective_merchant_id, Yii::app()->language);
            $items = CMerchantMenu::getMenu($effective_merchant_id, Yii::app()->language);
            $items_not_available = CMerchantMenu::getItemAvailability($effective_merchant_id, date("w"), date("H:i"));
            $category_not_available = CMerchantMenu::getCategoryAvailability($effective_merchant_id, date("w"), date("H:i"));
            ...
            $this->msg = "OK";
            $this->details = array(
                'merchant_id' => $merchant_id, // Original merchant ID
                'effective_merchant_id' => $effective_merchant_id, // Effective merchant ID (parent ID if CHILD)
                'data' => $data,
            );
        } catch (Exception $e) {
            $this->msg = t($e->getMessage());
    ...
    public function actionSimilarItems()
    {
        try {
            ...
            // Determine if the merchant is a CHAIN or CHILD
            $merchant = CMerchants::get($merchant_id);
            if (!$merchant) {
                throw new Exception("Merchant not found.");
            }

            // Determine if the merchant is part of a chain
            $is_chain = in_array($merchant['type'], ['CHAIN', 'SUPERMARKET_CHAIN', 'STORE_CHAIN']);
            $is_child = ($merchant['type'] == 'CHILD');
            ...
            $items = CMerchantMenu::getSimilarItemsByMerchantIds($merchant_ids_to_search,Yii::app()->language,100,'',$items_not_available,$category_not_available);
            $this->code = 1; $this->msg = "OK";
            $this->details = array(
                'data'=>$items
            );
    ...
    public function actionPlaceOrder()
    {
        ...
        if(!empty($payment_method) && $payment_method->payment_code == 'cybersource'){
            ...
            if($payment_method->attr3){
                $payment_token = $payment_method->attr3;
            }
        }
        $local_id = isset($this->data['local_id'])?$this->data['local_id']:(CommonUtility::getCookie(Yii::app()->params->local_id) ?? '');
        $cart_uuid = isset($this->data['cart_uuid'])?trim($this->data['cart_uuid']):'';
        ...
        $currency_code = isset($this->data['currency_code'])?trim($this->data['currency_code']):'';
        $base_currency = Price_Formatter::$number_format['currency_code'];
        $from = isset($this->data['from']) ? trim($this->data['from']) : '';
        ...
        try {
            ...
            try {
                $client_id = Yii::app()->user->id ?? 0; // Handle guest users
                $applicable_discount = Yii::app()->db->createCommand()
                    ->select('*')
                ...
                $merchant_discount_amount = $applicable_discount['merchant_discount_amount'] ?? 0;
                $system_discount_amount = $applicable_discount['system_discount_amount'] ?? 0;
                $city_discount_percentage = $applicable_discount['city_discount_percentage'] ?? 0;
                $merchant_forced = $applicable_discount['merchant_free_delivery_forced'] ?? 0;
                $system_forced = $applicable_discount['system_discount_is_forced'] ?? 0;

            } catch (Exception $e) {
                $this->msg[] = t("Unable to fetch applicable discounts.");
                $this->responseJson();
            }
        ...
        try {
            ...
            if(is_array($error) && count($error)>=1){
                ...
                if ($merchant_forced && $system_forced) {
                    // Both forced - Merchant takes priority
                    $total_discount += $merchant_discount_amount;
                } elseif ($merchant_forced) {
                    $total_discount += $merchant_discount_amount;
                } elseif ($system_forced) {
                    $total_discount += $system_discount_amount;
                } else {
                    // No forced discount, apply both if available
                    $total_discount += $merchant_discount_amount + $system_discount_amount;
                }

                // Apply city discount on top if available
                if ($city_discount_percentage > 0) {
                    $city_discount = ($total - $total_discount) * ($city_discount_percentage / 100);
                    $total_discount += $city_discount;
                }
                ...
                if($model->save()){
                    ...
                    $this->details = array(
                        ...
                        'discount_breakdown' => [
                            'merchant_discount' => $merchant_discount_amount,
                            'system_discount' => $system_discount_amount,
                            'city_discount' => $city_discount_percentage
                        ],
                    );
                } else {
    ...
    public function actiontag_list()
    {
        $stmt = '
            SELECT * FROM st_tags
        ';
        $tags_list = Yii::app()->db->createCommand($stmt)->queryAll();

        $tag_list = array_map(function($tag) {
            $tag['type'] = $tag['type'] == 1? 'Restaurant' : ($tag['type'] == 2? 'Market': 'Home Cooking');
            return $tag;
        }, $tags_list);
    ...
    public function actionPayment()
    {
        if(!Yii::app()->user->isGuest){
            ...
            $order_uuid = Yii::app()->input->post('order_uuid');
            $amount = Yii::app()->input->post('amount');
            $currency = Yii::app()->input->post('currency');
            $cybersourceToken = Yii::app()->input->post('cybersourceToken');

            if (empty($order_uuid)) {
                $this->code = 0;
                $this->msg = "Order id are required";
                $this->responseJson();
            }

            if (empty($amount)) {
                $this->code = 0;
                $this->msg = "Amount required";
                $this->responseJson();
            }

            if (empty($currency)) {
                $this->code = 0;
                $this->msg = "Currency required";
                $this->responseJson();
            }
            ...
            if ($order->payment_status == 'paid') {
                $this->code = 0;
                $this->msg = "Order has already been paid";
                $this->responseJson();
            }
            ...
            $params = array_merge([
                'access_key' => $this->accessKey,
                'profile_id' => $this->profileId,
                'transaction_uuid' => "{$randomNumber}::{$order->order_uuid}",
                "signed_field_names" => "access_key,profile_id,transaction_uuid,signed_field_names,unsigned_field_names,signed_date_time,locale,transaction_type,reference_number,amount,currency",
                "unsigned_field_names" => "",
                'signed_date_time' => gmdate('Y-m-d\TH:i:s\Z'),
                'locale' => 'en',
            ...
            if(isset($cybersourceToken) && $cybersourceToken){
                $params['transaction_type'] = 'sale';
                $params['payment_token'] = $cybersourceToken;
                $params['signed_field_names'] .= ",payment_token";
            }

            $params["signature"] = $this->generateSignature($params);

            $getSignYummyKey = $this->hmacParameters($params);

            $link = Yii::app()->getRequest()->getBaseUrl(true).'/interface/checkouturl?key='.$getSignYummyKey;
    ...
    public function actionCheckoutUrl()
    {
        $data = Yii::app()->input->get('key');
        $data = base64_decode($data);

        $data = json_decode($data, true);

        if (isset($data['payment_token']) && $data['payment_token']) {
            $endpoint = CYBERSOURCE_ONECLICK_ENDPOINT;
        } else {
            $endpoint = CYBERSOURCE_ENDPOINT;
        }
        ...
        $html .= '<title>Secure Acceptance Payment</title>';
        $html .= '</head>';
        $html .= '<body>';
        $html .= '<h1>';
        $html .= 'Please wait while we redirect you to the payment page...';
        $html .= '</h1>';
        $html .= '<form id="yummy_payment_form" action="'.$endpoint.'" method="post">';
        foreach ($data as $key => $value) {
            $html .= '<input type="hidden" name="'.$key.'" value="'.$value.'">';
        }
        $html .= '<input type="submit" style="display: none;" value="Pay Now">';
        $html .= '</form>';
        $html .= '<script>';
        $html .= 'document.getElementById("yummy_payment_form").submit();';
        $html .= '</script>';
        $html .= '</body>';        
        $html .= '</html>';
    ...
    public function actionOrderstatus()
    {
        ...
        if (!(isset($order->order_id) && $order->order_id)) {
            $this->code = 0;
            $this->msg = "Order with that order id does not exist";
            $this->details = [];
            $this->responseJson();
        }

        $payment = AR_ordernew_transaction::model()->find('order_id=:order_id', [':order_id'=> $order->order_id]);

        if (!(isset($payment->transaction_uuid) && $payment->transaction_uuid)) {
            $this->code = 0;
            $this->msg = "Transaction for this order was not registered";
            $this->details = [];
            $this->responseJson();
        }

        $this->code = 1;
        $this->msg = "Order details";
        $this->details = [
            'order_id' => $order->order_id,
    ...
    {
        $decision = Yii::app()->input->post('decision');
        $req_currency = Yii::app()->input->post('req_currency');
        $req_amount = Yii::app()->input->post('req_amount');
        $transaction_id = Yii::app()->input->post('req_transaction_uuid');
        $reason_code = Yii::app()->input->post('reason_code');
        $resason_message = Yii::app()->input->post('message');

        $txid = explode('::', $transaction_id);
        $transaction_id =  $txid[1];

        // Verify signature
        $verifysignature = $this->verifyResponseSignature($_POST, $this->secretKey);

        if (!$verifysignature) {
            Yii::log("Signature verification failed", "info");

            $this->code = 0;
            $this->msg = "Signature verification failed";
            $this->details = [];
            $this->responseJson();
        }
        
        
        if (isset($decision) && $decision === 'ACCEPT') {

            $order = AR_ordernew::model()->find('order_uuid=:order_uuid', [':order_uuid'=> $transaction_id]);

            $this->savedTransaction($order, [
                'order_id' => $order->order_id,
                'merchant_id' => $order->merchant_id,
                'client_id' => $order->client_id,
                'cart_uuid' => $order->cart_uuid,
                'total' => $req_amount, //amount from cybersource,
                'payment_code' => $order->payment_code,
            ]);

            Yii::log("Payment Successful!", "info");            
            $this->code = 1;
            $this->msg = "Payment Successful!";
            $this->details = [];
            $this->responseJson();
            ...
            $message = "Payment Failed or Declined! Reason Code: ";
            $reason = '';

            if (isset($reason_code) && $reason_code) {
                $resason_message = isset($resason_message) && $resason_message ? ' Message: ' . $resason_message : '';
                $reason = $reason_code.$resason_message;
            }
    ...
    private function verifyResponseSignature($responseParams, $secretKey)
    {
        $signedFields = $responseParams['signed_field_names'];

        $signedData = [];
        $fields = explode(',', $signedFields);
        foreach ($fields as $field) {
            if (isset($responseParams[$field])) {
                $signedData[] = $field . "=" . $responseParams[$field];
            }
        }
        $signedString = implode(",", $signedData);

        $generatedSignature = base64_encode(hash_hmac("sha256", $signedString, $secretKey, true));

        if ($generatedSignature === $responseParams['signature']) {
            return true;
        } else {
            return false;
        }
    ...
    private function hmacSignData($params)
    {
        ...
        $params["signature"] = base64_encode(hash_hmac("sha256", $signedString, $secretKey, true));

        return $params;
    }


    private function hmacParameters($params)
    {
        return base64_encode(json_encode($params));
    }

    private function savedTransaction($order, $data)
	{
		if($order){
			$order->scenario = "new_order";
			$order->status = COrders::newOrderStatus();
			$order->cart_uuid = $data['cart_uuid'] ?? null;
    ...
    public function actionCityBoundaries()
    {
        try {
            ...
            $boundaries = ClocationCountry::getCityBoundaries($city_id);

            $rdata = [
                'status' => "success",
                'bound_points' => $boundaries
            ];
    ...
    public function actiongetLocationStates()
    {
        try {
            ...
            $states = ClocationCountry::getStatesByCountry($country_id);
    ...
    public function actiongetLocationCities()
    {
        try {
            ...
            $cities = ClocationCountry::getCitiesByState($state_id);
    ...
    public function actiongeStoreCategory()
    {
        try {
            $slug = Yii::app()->input->post('slug');
            $model = CMerchantListingV1::getMerchantBySlug($slug);
    ...
    public function actiongeStoreItems()
    {
        try {
            $slug = Yii::app()->input->post('slug');
            $category_id = Yii::app()->input->post('category_id'); // or UUID
            $currency_code = Yii::app()->input->post('currency_code');
            ...
            $items = CMerchantMenu::getMenu($merchant_id, Yii::app()->language, $category_id);
            $items_not_available = CMerchantMenu::getItemAvailability($merchant_id, date("w"), date("H:h:i"));
            $category_not_available = CMerchantMenu::getCategoryAvailability($merchant_id, date("w"), date("H:h:i"));
            $dish = CMerchantMenu::getDish(Yii::app()->language);
    ...
    public function actionGetMerchantTypes()
    {
        try {
            $place_id = Yii::app()->input->post('place_id');
            ...
            $place_data = CMaps::locationDetails($place_id, '');
            if (!isset($place_data['latitude']) || !isset($place_data['longitude'])) {
                throw new Exception("Invalid place ID or missing coordinates.");
            }

            $latitude = $place_data['latitude'];
            $longitude = $place_data['longitude'];

            $zone_ids = AttributesTools::getZonesIDS(['lat' => $latitude, 'lon' => $longitude]);
            if (empty($zone_ids)) {
                throw new Exception("No zones found for the provided location.");
            }

            $criteria = new CDbCriteria();
            $criteria->distinct = true;
            $criteria->select = "type";
            $criteria->join = "JOIN {{merchant_meta}} mm ON t.merchant_id = mm.merchant_id";

            $placeholders = [];
            foreach ($zone_ids as $index => $zone_id) {
                $placeholders[] = ':zone_id_' . $index;
            }
            ...
            $criteria->addCondition("t.type != 'CHILD'");

            $merchant_types = AR_merchant::model()->cache(Yii::app()->params->cache, CCacheData::dependency())->findAll($criteria);
            ...
            $criteria = new CDbCriteria();
            $criteria->select = "t.type AS child_type, p.type AS parent_type";
            $criteria->join = "
                JOIN {{merchant_meta}} mm ON t.merchant_id = mm.merchant_id
                LEFT JOIN {{merchant}} p ON t.parent_id = p.merchant_id
            ";
            $criteria->addCondition("t.type = 'CHILD' AND p.type IS NOT NULL");
            ...
            $sql = "SELECT " . $criteria->select . " FROM {{merchant}} t " . $criteria->join;
            if (!empty($criteria->condition)) {
                $sql .= " WHERE " . $criteria->condition;
            }

            Yii::log("Child Query: " . $sql, CLogger::LEVEL_INFO);
            Yii::log("Child Params: " . print_r($criteria->params, true), CLogger::LEVEL_INFO);

            $child_records = Yii::app()->db->cache(Yii::app()->params->cache, CCacheData::dependency())
                ->createCommand($sql)
            ...
            if (!empty($child_records)) {
                foreach ($child_records as $record) {
                    if (!in_array($record['parent_type'], $types)) {
                        $types[] = $record['parent_type'];
                    }
                }
            }
            ...
            $this->msg = "OK";
            $this->details = [
                'merchant_types' => array_unique($types),
            ];
        } catch (Exception $e) {
            $this->msg = t($e->getMessage());
        }
