...
class CCheckout
{
	...
	public static function getMerchantTransactionList($merchant_id='',$lang='',$meta_name='services')
	{
		$stmt="
		SELECT 
		a.service_id,
		...
		if($res){
			$data = array();
			foreach ($res as $val) {		
				$val['service_name'] = empty($val['service_name']) ? $val['original_service_name'] : CHtml::decode($val['service_name']);
