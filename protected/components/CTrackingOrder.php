...
class CTrackingOrder
{
	   ...
	   ))
	{
		...
		$tracking_stats = AR_admin_meta::getMeta(array(
		 ...
		 'status_new_order','status_prepending_order','status_with_delivery_order','status_cancel_order','status_rejection','status_delivered','tracking_status_ready',
		...
		$status_prepending_order = isset($tracking_stats['status_prepending_order'])?AttributesTools::cleanString($tracking_stats['status_prepending_order']['meta_value']):'';
		$status_with_delivery_order = isset($tracking_stats['status_with_delivery_order'])?AttributesTools::cleanString($tracking_stats['status_with_delivery_order']['meta_value']):'';
		...
		// dump($status_new_order);
		// dump($tracking_status_process);
		// dump($tracking_status_ready);
    	
    	if($status == $status_new_order || $status == $status_prepending_order){
    		...
    		$order_progress = 2;
    	} elseif ( $status == $tracking_status_ready || $status == $status_with_delivery_order){
