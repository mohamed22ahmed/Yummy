...
class CCart
{
    ...
    public static function getCart($cart_uuid='', $lang = KMRS_DEFAULT_LANGUAGE)
    {
		...
		AND (b.merchant_id =".q($merchant_id)." OR FIND_IN_SET(".q($merchant_id).", b.locations))
		ORDER BY id ASC
		";
                        ...
                        if($val['type']=="tax"){
                            ...
                            if($val['type'] == 'delivery_fee') {
                                ...
                                $results[] = array(
                                    ...
                                    'raw' => isset($value['raw']) ? $value['raw'] : 0,
                                    'type' => $val['type'],
                                    'pay' => Price_Formatter::formatNumber($pay),
                                );
                                $sub_total = CCart::apply($sub_total, $val['value']);
                            }elseif ($val['type'] == 'merchant_discount') {
                                $merchant_id = CCart::getMerchantId($cart_uuid);
                                [$up_to, $minimum_amount] = CCart::getUpToAndMinimumCartForMerchantDiscount($merchant_id);

                                if($sub_total >= $minimum_amount){
                                    $up_to *= -1;
                                    $results[] = array(
                                        'name' => $val['name'],
                                        'value' => '('.Price_Formatter::formatNumber($up_to*-1).')',
                                        'raw' => $up_to,
                                        'type' => $val['type'],
                                    );
                                    $sub_total = CCart::apply($sub_total, $up_to);
                                }
                            }else{
                                $results[] = array(
                                    'name' => $val['name'],
    ...
    public static function getUpToAndMinimumCartForMerchantDiscount($merchant_id){
        ...
        if($total_order_count == 0){
            $free_delivery_on_first_order = AR_option::model()->find('merchant_id=:merchant_id AND option_name=:option_name',
                array(
                    ':merchant_id'=>$merchant_id,
                    ':option_name'=>'free_delivery_on_first_order',
                ));
            if($free_delivery_on_first_order->option_value == 1)
            {
                $first_order_free_delivery_up_to_amount = AR_option::model()->find('merchant_id=:merchant_id AND option_name=:option_name',
                    array(
                        ':merchant_id'=>$merchant_id,
                        ':option_name'=>'first_order_free_delivery_up_to_amount',
                    ));

                $first_order_free_delivery_minimum_cart_amount = AR_option::model()->find('merchant_id=:merchant_id AND option_name=:option_name',
                    array(
                        ':merchant_id'=>$merchant_id,
                        ':option_name'=>'first_order_free_delivery_minimum_cart_amount',
                    ));

                return [$first_order_free_delivery_up_to_amount->option_value, $first_order_free_delivery_minimum_cart_amount->option_value];
            }
        }elseif ($total_order_count == 1) {
            $free_delivery_on_second_order = AR_option::model()->find('merchant_id=:merchant_id AND option_name=:option_name',
                array(
                    ':merchant_id'=>$merchant_id,
                    ':option_name'=>'free_delivery_on_second_order',
                ));
            if($free_delivery_on_second_order->option_value == 1)
            {
                $second_order_free_delivery_up_to_amount = AR_option::model()->find('merchant_id=:merchant_id AND option_name=:option_name',
                    array(
                        ':merchant_id'=>$merchant_id,
                        ':option_name'=>'second_order_free_delivery_up_to_amount',
                    ));

                $second_order_free_delivery_minimum_cart_amount = AR_option::model()->find('merchant_id=:merchant_id AND option_name=:option_name',
                    array(
                        ':merchant_id'=>$merchant_id,
                        ':option_name'=>'second_order_free_delivery_minimum_cart_amount',
                    ));

                return [$second_order_free_delivery_up_to_amount->option_value, $second_order_free_delivery_minimum_cart_amount->option_value];
            }
        }elseif ($total_order_count == 2) {
            $free_delivery_on_third_order = AR_option::model()->find('merchant_id=:merchant_id AND option_name=:option_name',
                array(
                    ':merchant_id'=>$merchant_id,
                    ':option_name'=>'free_delivery_on_third_order',
                ));
            if($free_delivery_on_third_order->option_value == 1)
            {
                $third_order_free_delivery_up_to_amount = AR_option::model()->find('merchant_id=:merchant_id AND option_name=:option_name',
                    array(
                        ':merchant_id'=>$merchant_id,
                        ':option_name'=>'third_order_free_delivery_up_to_amount',
                    ));

                $third_order_free_delivery_minimum_cart_amount = AR_option::model()->find('merchant_id=:merchant_id AND option_name=:option_name',
                    array(
                        ':merchant_id'=>$merchant_id,
                        ':option_name'=>'third_order_free_delivery_minimum_cart_amount',
                    ));

                return [$third_order_free_delivery_up_to_amount->option_value, $third_order_free_delivery_minimum_cart_amount->option_value];
            }
        }
    ...
    public static function getDistanceOld($cart_uuid='',$merchant_id='',$current_place_id='')
    {
        ...
        if($res = Yii::app()->db->createCommand($stmt)->queryRow()){
            ...
            if($resp = MapSdk::getOrSaveCachedDistance()){
                $resp['found']=false;
    ...
    public static function getDistance($client_id='', $place_id='',$unit='',$merchant_lat='',$merchant_lng='')
    {
        ...
        try {
            ...
            $distance =  MapSdk::getOrSaveCachedDistance();
    ...
    {
        $stmt = "
		SELECT a.item_token, b.item_name as original_item_name, c.item_name
		FROM {{cart}} a
		LEFT JOIN {{item}} b
        ...
        if($res = Yii::app()->db->cache(Yii::app()->params->cache, $dependency)->createCommand($stmt)->queryAll()){
            $data = []; $item_line='';
            foreach ($res as $items) {
                $item_name = empty($items['item_name'])?$items['original_item_name']:$items['item_name'];
                $item_line.= "$item_name,";
                $data[] = [
